...
class Admin_UI
{
	 ...
	 */
	public $class_name = 'WP_Predictive_Search';

	public $plugin_prefix = WPPS_PREFIX;

	/**
	 * @var string
	 * You must change to correct pro plugin page url on a3rev site
	 */
	public $pro_plugin_page_url = 'https://a3rev.com/shop/wp-predictive-search/';
	...
	public function __construct() {
		$this->google_api_key_option     = WPPS_KEY . '_google_api_key';
		$this->google_map_api_key_option = WPPS_KEY . '_google_map_api_key';
		$this->toggle_box_open_option    = WPPS_KEY . '_toggle_box_open';
		$this->version_transient         = WPPS_KEY . '_licinfo';

		if ( defined( 'WPPS_G_FONTS' ) ) {
	...
	public function get_premium_video_data() {
		$premium_video_data = array(
				'box_title'    => __( 'Premium Version Enhanced Features', 'wp-predictive-search' ),
				'image_url'    => WPPS_IMAGES_URL. '/video.jpg',
				'video_url'    => 'https://www.youtube.com/embed/9dGw-ORfMIk?version=3&autoplay=1',
				'left_title'   => __( 'Premium Version Enhanced Features', 'wp-predictive-search' ),
				'left_text'    => __( 'WordPress Predictive Search Premium', 'wp-predictive-search' )
									. "\n\n" . __( 'Quick Video showing the main (not all) enhanced features that are built into the WordPress Predictive Search Premium version', 'wp-predictive-search' ),
				'right_title'  => __( 'Developer Support and Premium Features', 'wp-predictive-search' ),
				'right_text'   => __( 'Limited Time Offer. Purchase the Premium Version Lifetime License. That is a Lifetime of maintenance updates, feature upgrades and developer support for a once only fee. Offer ending soon.', 'wp-predictive-search' )
	...
	public function upgrade_top_message( $echo = false, $setting_id = '' ) {
		$upgrade_top_message = sprintf( '<div class="pro_feature_top_message">' 
			. __( 'Advanced Settings - Upgrade to the <a href="%s" target="_blank">%s License</a> to activate these settings.', 'wp-predictive-search' ) 
			. '</div>'
			...
			, apply_filters( $this->plugin_name . '_' . $setting_id . '_pro_version_name', apply_filters( $this->plugin_name . '_pro_version_name', __( 'Pro Version', 'wp-predictive-search' ) ) )
		);
	...
	public function get_version_message() {
		...
		if ( false !== $version_transient ) {
			...
			if ( false === $timeout ) {
				$version_message = __( 'You should check now to see if have any new version is available', 'wp-predictive-search' );
			} elseif ( 'cannot_connect_api' == $version_transient ) {
				$version_message = sprintf( __( 'Connection Failure! Please try again. If this issue persists please create a support request on the plugin <a href="%s" target="_blank">a3rev support forum</a>.', 'wp-predictive-search' ), $this->support_url );
			} else {
				...
				if ( FALSE !== stristr( $version_transient, '||' )
					...
					&& version_compare( WPPS_VERSION , $version_info[0], '<' ) ) {

						$version_message = sprintf( __( 'There is a new version <span class="a3rev-ui-new-plugin-version">%s</span> available, <a href="%s" target="_blank">update now</a> or download direct from <a href="%s" target="_blank">My Account</a> on a3rev.com', 'wp-predictive-search' ),
							$version_info[0],
