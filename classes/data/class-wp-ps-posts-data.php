...
class Posts
{
	 ...
	 * @return void
	 */
	public function get_sql( $search_keyword = '', $search_keyword_nospecial = '', $post_type = 'post', $number_row = 6, $start = 0, $check_existed = false ) {
		...
		$items_excluded = apply_filters( 'wpps_items_excluded', $wpps_exclude_data->get_array_items( $post_type ), $post_type );

		if ( 'page' == $post_type ) {
			global $wpps_search_page_id;
			$items_excluded = array_merge( array( (int) $wpps_search_page_id ), $items_excluded );
		}

		$join = apply_filters( 'wpps_post_get_sql_join', $join, $search_keyword, $post_type );
		$where = apply_filters( 'wpps_post_get_sql_where', $where, $search_keyword, $post_type );
		...
		$where_title = ' ( ';
		$where_title .= WPPredictiveSearch\Functions::remove_special_characters_in_mysql( 'pp.post_title', $search_keyword );
		if ( '' != $search_keyword_nospecial ) {
			$where_title .= " OR ". WPPredictiveSearch\Functions::remove_special_characters_in_mysql( 'pp.post_title', $search_keyword_nospecial );
		}
		...
		if ( $search_keyword_no_s_letter != false ) {
			$where_title .= " OR ". WPPredictiveSearch\Functions::remove_special_characters_in_mysql( 'pp.post_title', $search_keyword_no_s_letter );
		}
		$where_title .= ' ) ';
