...
import SearchInputAttributes from './attributes';
...
const typographyAttributes = TypographyAttributes( 'typo' );
 ...
 */
registerBlockType( 'wp-predictive-search/search-input', {
	// Block name. Block names must be string that contains a namespace prefix. Example: my-plugin/my-custom-block.
	title: __( 'Search Input' ), // Block title.
	apiVersion: 2,
	icon: {
		src: IconChild,
		foreground: '#24b6f1',
	...
	save() {
		return (
			<IconLoading aria-label={ __( 'Searching' ) } className={ 'wp-block-wp-predictive-search-search-input wpps_searching_icon' } />
		);
	},
} );
